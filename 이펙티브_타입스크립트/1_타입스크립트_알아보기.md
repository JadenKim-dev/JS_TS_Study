...
js 런타임에서의 타입과 ts에서 정의한 타입은 불일치할 수 있다.  
...
하지만 타입스크립트의 타입 정보는 런타임에 제거되기 때문에 함수를 오버라이딩하는 것이 불가능하다.  
...
자바스크립트로 변환하는데 소요되는 빌드 타임 오버헤드만 존재하며, 컴파일 성능도 최적화되어 있기 때문에 빠르게 컴파일이 가능하다.  
...
함수 호출 시 매개변수에 선언된 속성만 넘어올 것이라고 가정하기 쉽다.  
...
이는 Vector3D의 정의된 속성은 모두 number이지만, 다음과 같이 다른 타입의 값이 추가된 형태로 호출될 수도 있기 때문이다.
...
이러한 특성으로 인해 타입스크립트에서는 클래스 타입으로 변수를 선언하더라도, 해당 변수가 해당 클래스 또는 서브클래스의 인스턴스임을 보장하지 않는다.
